...
pub fn build(b: *std.Build) void {
    ...
    //     .name = "zig-tcp-command-server.nosync",
    ...
    // such a dependency.
    const server_run_cmd = b.addRunArtifact(serverexe);
    const client_run_cmd = b.addRunArtifact(clientexe);
    ...
    const server_run_step = b.step("server", "Run the server");
    server_run_step.dependOn(&server_run_cmd.step);

    const client_run_step = b.step("client", "run the client");
